...
The CN field could be any string. The "hosts" field, by default it is ```["127.0.0.1","localhost"]```. The above configuration includes Application server’s public IP here as follows: ```["127.0.0.1","localhost","192.168.1.3"]```
...
The JS also will act as ```Client``` when trying to authenticate the NS and the JS will act as ```Server``` when the NS is trying to authenticate it. Both the communications for authentications are done by [API]. Hence there are two groups of certificates needed:
...
Modify ```certificate.json``` is adapted to fit your Network Server deployment.
